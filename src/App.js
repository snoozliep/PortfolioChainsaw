...
function App() {
  ...
  const [bubbles, setBubbles] = useState(
    Array.from({ length: 20 }).map((_, i) => ({
      ...
      duration: Math.random() * 10 + 10, // 10-20s duration for variety
      randomX: Math.random() * 200 - 100, // -100 to 100px horizontal drift
    }))
  );

  // Render bubbles (floating around, no clicking)
  const bubbleElements = bubbles.map((bubble) => (
    <div
      ...
      style={{
        ...
        '--random-x': `${bubble.randomX}px`, // Random horizontal movement
      }}
    />
  ));
  ...
  return (
    <>
      ...
      >
        {/* Flashlight effect overlay in dark mode, only when skull is clicked */}
